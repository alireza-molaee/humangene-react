...
export function getCategories() {
  return ajaxJson.get("/cms/category").then(res => {
...
export function getPages() {
  return ajaxJson.get("/cms/page").then(res => {
    return res.data.map(item => ({
      id: item["id"],
      title: item["title"],
...
export function getHeaderItems() {
  return axios.all([getCategories(), getPages()]).then(
    axios.spread((categories, pages) => {
      let headerItems = [];
      const categoryMixedPage = categories
        .map(cat => {
      ...
      const pageWithNoCategory = pages
...
export function getArticleDetail(id) {
  return ajaxJson.get(`/cms/page/${id}`).then(res => {
    return {
      id: res.data["id"],
...
export function getPostList() {
  return ajaxJson.get(`/cms/news`).then(res => {
    return res.data.map(item => {
      return {
        id: item["id"],
        title: item["title"],
...
export function getPostSingle(id) {
  return ajaxJson.get(`/cms/news/${id}`).then(res => {
    return {
      id: res.data["id"],
      title: res.data["title"],
      image: res.data["image"],
...
export function getPostComents(id) {
  return ajaxJson.get(`cms/news/${id}/comment`).then(res => {
    return res.data.map(item => {
      return {
        id: item["id"],
        text: item["text"],
...
export function getServices() {
  return ajaxJson.get(`/cms/service`).then(res => {
    return res.data.map(item => {
      return {
        title: item["title"],
        image: item["image"],
...
export function getFeatures() {
  return ajaxJson.get(`/cms/feature`).then(res => {
    return res.data.map(item => {
      return {
        title: item["title"],
        icon: item["icon"],
...
export function getCompanies() {
  return ajaxJson.get(`/cms/companies`).then(res => {
    return res.data.map(item => ({
      title: item["title"],
      image: item["image"],
      link: item["link"]
...
export function getSlids() {
  return ajaxJson.get("/cms/slider").then(res => {
    if (!res.data[0]) {
      return [];
    }
...
export function getHomeData() {
  return axios
    .all([getSlids(), getFeatures(), getServices(), getCompanies()])
    .then(
      axios.spread((slids, features, services, companies) => {
        return {
          slids,
          features,
          services,
...
export function getMemberData() {
  return ajaxJson.get("/cms/members").then(res => {
    return res.data.map(item => ({
      image: item["image"],
      name: item["name"],
      job: item["job"],
